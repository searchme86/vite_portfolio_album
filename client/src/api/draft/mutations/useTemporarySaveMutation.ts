/**
 * @file useTemporarySaveMutation.ts
 * @description React Query를 사용하여 임시저장 mutation을 처리하는 커스텀 훅
 * @reason 임시저장 mutation 로직을 분리하여 단일 책임 원칙 준수
...
import temporarySaveAxios from '../axios/temporarySaveAxios'; // @type {Function} - 임시저장 API 요청 함수
// @description 임시저장 API 요청 함수 가져오기
// @reason 임시저장 요청 실행
// @analogy 도서관에서 임시저장 시스템 연결
...
// API 응답 타입 정의 (temporarySaveAxios.ts와 동일)
...
const useTemporarySaveMutation = () => {
  ...
  // @reason 애플리케이션 충돌 방지
  const handleTemporarySave = (draftData: DraftData) => {
    if (!draftData.draftId) {
      console.warn('useTemporarySaveMutation - Draft ID is missing'); // @description 경고 로그
  ...
  return {
    temporarySave: handleTemporarySave, // @type {Function} - 임시저장 함수
    isLoading: mutation.isLoading, // @type {boolean} - 로딩 상태
    error: mutation.error, // @type {Error | null} - 에러 상태
    data: mutation.data, // @type {TemporarySaveResponse | undefined} - 응답 데이터
...
// @reason 컴포넌트에서 임시저장 mutation 사용 가능
// @analogy 도서관에서 임시저장 시스템을 공유
export default useTemporarySaveMutation;
