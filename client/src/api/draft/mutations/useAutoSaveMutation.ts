...
const useAutoSaveMutation = (
  ...
  const mutation = useMutation<AutoSaveResponse, Error, DraftData>({
    mutationFn: async (draftData: DraftData) => {
      ...
      console.log('useAutoSaveMutation - Retrieved token:', token); // @description 토큰 디버깅
      // @reason 토큰 상태 확인

      console.log('useAutoSaveMutation - Mutation params:', {
        draftData,
        token,
        isAuthenticated,
      ...
      const response = await autoSaveAxios(draftData, token); // @description autoSaveAxios 호출
      // @reason 토큰과 함께 자동저장 요청 실행
      console.log('useAutoSaveMutation - Mutation response:', response); // @description 응답 디버깅
      // @reason 응답 상태 확인

      return response; // @type {AutoSaveResponse} - 응답 반환
...
// 6. `mutationFn`으로 `autoSaveAxios` 호출: 토큰과 함께 자동저장 API 요청 실행.
// 7. `onSuccess`, `onError`, `onSettled` 콜백 정의: mutation 상태 디버깅.
// 8. `handleAutoSave` 함수 정의: mutation 실행 및 Fallback 처리.
// 9. `autoSave`, `isLoading`, `error`, `data` 반환: 컴포넌트에서 상태와 함수 사용 가능.
