import { deleteTagFromPostService } from '../services/tag.deleteTagFromPost.service.js'; // 의미: 태그 삭제 서비스 가져오기
// 이유: 데이터 수정 로직을 서비스로 분리
// 비유: 도서관에서 책 주제를 삭제하는 전문가 호출
...
export const deleteTagFromPost = async (req, res) => {
  // 의미: 특정 포스트에서 태그를 삭제하는 컨트롤러 함수
  // 이유: 클라이언트 요청 처리 및 응답 제공
  // 비유: 도서관 데스크에서 손님 요청 처리
  const { postId, tag } = req.params; // 의미: URL 파라미터에서 postId와 tag 추출
  // 이유: 삭제할 포스트와 태그 식별
  // 비유: 손님이 말한 책 번호와 삭제할 주제를 확인

  console.log(`deleteTagFromPost - Deleting tag: ${tag} from post: ${postId}`); // 의미: 요청된 태그와 postId 로그
  // 이유: 디버깅 용이성
  // 비유: 손님이 원하는 책 번호와 삭제할 주제를 기록
  ...
  try {
    ...
    if (!validationResult.isValid) {
      ...
      console.log(
        'deleteTagFromPost - Validation failed:',
    ...
    const updatedPost = await deleteTagFromPostService(postId, tag); // 의미: 서비스에서 태그 삭제
    // 이유: 데이터 수정 로직을 서비스에 위임
    // 비유: 전문가에게 책에서 주제 삭제 요청

    console.log(`deleteTagFromPost - Tag deleted: ${tag} from post: ${postId}`); // 의미: 태그 삭제 성공 로그
    // 이유: 디버깅 용이성
    // 비유: 주제 삭제 성공 확인

    return res.status(200).json(updatedPost); // 의미: 업데이트된 포스트 반환
    // 이유: 클라이언트에 성공 응답
    // 비유: 손님에게 주제가 삭제된 책 전달
  } catch (error) {
    console.error('deleteTagFromPost - Error:', error.message); // 의미: 에러 로그
    // 이유: 문제 원인 추적
    // 비유: 문제 발생 시 일지에 기록
