...
// 자동저장 서비스 함수
// @description 드래프트 데이터를 받아 자동저장 처리
// @reason 데이터베이스에 드래프트 데이터를 저장
// @analogy 도서관에서 자료를 자동저장
const autoSaveDraftService = async (userId, draftData) => {
  try {
    ...
    const draftToSave = {
      ...
      // @description 임시저장 플래그 설정, 기본값 false
    ...
    if (existingDraft) {
      ...
      // @reason 기존 데이터 갱신
      console.log('autoSaveDraftService - Draft updated:', savedDraft); // @description 디버깅용 로그
      ...
      // @reason 새 데이터 저장
      console.log('autoSaveDraftService - Draft created:', savedDraft); // @description 디버깅용 로그
    ...
    throw new Error(error.message || 'Failed to auto-save draft'); // @description 에러 던지기
    // @reason 컨트롤러에서 에러 처리
  }
};
...
// 7. `export`로 서비스 내보내기: `draft.autoSaveDraft.controller.js`에서 사용 가능.
// @reason 자동저장 비즈니스 로직을 캡슐화하여 코드 재사용성과 유지보수성 향상.
// @analogy 도서관에서 자료를 자동저장하는 시스템.
