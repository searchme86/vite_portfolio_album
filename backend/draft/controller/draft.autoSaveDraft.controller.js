...
import { autoSaveDraftService } from '../service/draft.autoSaveDraft.service.js'; // @type {Function} - 자동저장 서비스
// @description 자동저장 서비스 가져오기
// @reason 비즈니스 로직 처리
// @analogy 도서관에서 자료 저장 담당 부서 연결

// 자동저장 컨트롤러 함수
// @description 클라이언트의 자동저장 요청 처리
// @reason 드래프트 데이터를 받아 저장 후 응답
// @analogy 도서관에서 자료 자동저장 요청 접수
const autoSaveDraftController = async (req, res) => {
  try {
    console.log('autoSaveDraftController - Request received:', req.body); // @description 디버깅용 로그
    ...
    if (!userId) {
      console.error('autoSaveDraftController - User ID is missing'); // @description 인증 실패 로그
      // @reason 문제 원인 추적
    ...
    // 서비스 호출하여 드래프트 저장
    // @description autoSaveDraftService로 데이터 저장 요청
    // @reason 비즈니스 로직 실행
    const savedDraft = await autoSaveDraftService(userId, draftData); // @type {Object} - 저장된 드래프트 데이터
    // @description 서비스에서 반환된 결과
    // @reason 저장 결과 반환

    console.log('autoSaveDraftController - Draft saved:', savedDraft); // @description 디버깅용 로그
    // @description 저장 성공 디버깅
    // @reason 성공 상태 확인

    // 성공 응답 반환
    // @description 클라이언트에 저장 성공 응답
    // @reason 요청 완료 알림
    ...
    // @description AutoSaveResponse 형식의 응답
    // @reason 클라이언트에 결과 전달
  } catch (error) {
    // 에러 로깅
    ...
    // @description AutoSaveResponse 형식의 기본 응답
...
// 4. `autoSaveDraftService` 호출: 사용자 ID와 드래프트 데이터를 전달하여 저장 요청.
// 5. 성공 응답 반환: `AutoSaveResponse` 형식으로 성공 응답 전달.
// 6. 에러 발생 시 처리: 에러 로깅 후 Fallback 응답 반환으로 충돌 방지.
// 7. `export`로 컨트롤러 내보내기: `draft.route.js`에서 사용 가능.
// @reason 자동저장 요청을 처리하는 컨트롤러 로직을 캡슐화하여 코드 재사용성과 유지보수성 향상.
// @analogy 도서관에서 자료를 자동저장하는 직원.
